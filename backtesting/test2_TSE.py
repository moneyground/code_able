...
for i, datetime in enumerate(df.index):
    if datetime.day % 2 == 1 and datetime.hour == 0 and datetime.minute == 0:
        day_list.append(i)
        name_list.append(datetime.strftime('%Y-%m-%d'))
...
# EMA 200
ema200 = list(df["close"].ewm(span=200, adjust=False).mean())
df["ema200"] = ema200

# SuperTrend
df['st'], df['st_up'], df['st_dt'] = get_supertrend(df['high'], df['low'], df['close'], 10, 3)
df = df[1:]
st = list(df["st"])
st_up = list(df["st_up"])
st_dt = list(df["st_dt"])
...
# entryPoint
buy_price , sell_price, entry_price = get_tse_entryPoint(df["open"], df["st"], df["ema200"])

# TakePrice Line
takeprofitline = get_takeprofit_line(entry_price, df["st"], 2)



ax.plot(ema200, label = "EMA 200")
ax.plot(st_up, color = 'green', linewidth = 1, label = 'ST UPTREND 10,3')
ax.plot(st_dt, color = 'r', linewidth = 1, label = 'ST DOWNTREND 10,3')
ax.plot(buy_price, marker = '^', color = 'green', markersize = 10, linewidth = 0, label = 'BUY SIGNAL')
ax.plot(sell_price, marker = 'v', color = 'r', markersize = 10, linewidth = 0, label = 'SELL SIGNAL')
ax.plot(takeprofitline, color = "gold", marker = "o", linewidth = 0.5, label ="TakePricePoint")
