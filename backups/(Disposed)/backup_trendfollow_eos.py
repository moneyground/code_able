...
def check_trend(exchange, symbol, time):
    ...
    if past5["ma5"] > past5["ma10"] > past5["ma15"] > past5["ma25"]:
        ...
        # 조건2 : 조건1을 만족하며 현재 분봉 데이터가 정배열? -> 정배열 추세
        if present["ma5"] > present["ma10"] > present["ma15"] > present["ma25"]:
            bot.sendMessage(chat_id = chat_id, text = "EOS 정배열 추세 포착됨!")
            print(time.strftime('%Y-%m-%d %H:%M'), "정배열 알람 전송됨!")

            # 조건1, 조건2를 만족하지만 ma5 기울기( (5분전 ma5 - 현재 ma5)/15 )가 음수인가? -> 정배열 돌파조짐
            if ma5_inclination < 0.002:
                bot.sendMessage(chat_id=chat_id, text="EOS 정배열 돌파조짐 포착됨!")
                print("정배열 돌파조짐 알람 전송됨!")

        # 조건1을 만족하지만 현재 분봉 기준 ma5<ma25 인가? -> 정배열 돌파
        if present["ma5"] < present["ma10"]:
            bot.sendMessage(chat_id = chat_id, text = "EOS 정배열 돌파 포착됨!")
    ...
    if past5["ma5"] < past5["ma10"] < past5["ma15"] < past5["ma25"]:
        ...
        # 조건2 : 조건1을 만족하면서 현재 분봉 데이터가 역배열인가? -> 역배열 추세
        if present["ma5"] < present["ma10"] < present["ma15"] < present["ma25"]:
            bot.sendMessage(chat_id = chat_id, text = "EOS 역배열 추세 포착됨!")
            print(time.strftime('%Y-%m-%d %H:%M'), "역배열 알람 전송됨!")

            # 조건1, 조건2를 만족하지만 ma5 기울기( (5분전 ma5 - 현재 ma5)/15 )가 -0.002보다 작은가? -> 역배열 돌파조짐
            if ma5_inclination > -0.002:
                bot.sendMessage(chat_id = chat_id, text = "EOS 역배열 돌파조짐 포착됨!")
                print("역배열 돌파조짐 알람 전송됨!")

        # 조건1을 만족하지만 현재 분봉 기준 ma5>ma25 인가? -> 역배열 돌파
        if present["ma5"] > present["ma10"]:
            bot.sendMessage(chat_id = chat_id, text = "EOS 역배열 돌파 포착됨!")
